...
from videos.models import Distribucion, Distribuido
...
def get_video_context(video):
    context = {
        'video_fecha': video.fecha.strftime('%Y-%m-%d'), 'video_tipo': video.tipo.nombre,
        'video_titulo': video.titulo, 'video_descripcion': video.descripcion,
        'video_thumbnail_url': video.thumbnail_grande(),
        'video_descarga_url': video.get_archivo_storage_url() + '?descargar',
        'video_duracion': str(video.duracion),
        'video_filesize': sizeof_fmt(video.archivo.size),
        'video_url': video.make_url(),
        'video_slug': video.slug
    }
    if video.programa: context.update(video_programa=video.progsrama.nombre)
    if video.categoria: context.update(video_categoria=video.categoria.nombre)
    if video.corresponsal: context.update(video_corresponsal=video.corresponsal.nombre)
    if video.hashtags: context.update(video_hashtags=video.hashtags)
    if video.tema: context.update(video_tema=video.tema.nombre)
    if video.serie: context.update(video_serie=video.serie.nombre)
    if video.pais: context.update(video_pais=video.pais.nombre)

    # static context
    if video.idioma_original == 'en':
        context.update({
        ...
        context.update({
            ...
            'static_saludo': u'Mediante este mensaje se le notifica la reciente publicación en nuestra plataforma del video de video titulado:',
            ...
            'static_enviado_porque': u'debido a que nuestras reglas de distribución automática de videos de video indican que usted así lo desea.',
...
def distribucion_job():
    ...
    for distribucion in Distribucion.objects.filter(activo=True):
        ...
        distribuidos_pks = distribucion.distribuidos.values_list('video__pk', flat=True)
        videos = distribucion.get_videos_distribuibles().exclude(pk__in=distribuidos_pks).order_by('fecha')
        print(u"Total videos seleccionados para distribuir: %s" % videos.count())

        if distribucion.email:
            for video in videos[:30]:
                print(u"Distribuyendo por email video: %s" % video)
                dist = Distribuido(status=1, video=video, distribucion=distribucion)
                sent = False

                context = get_video_context(video)

                if video.idioma_original == 'en':
                    subject = u'Video published notification: %s' % video.titulo
                else:
                    subject = u'Notificación de video publicado: %s' % video.titulo
                for email in distribucion.get_email_dict():
                    if email[1] == 'CORRESPONSAL':
                        if video.corresponsal and video.corresponsal.email:
                            print(u'Distribuyendo a corresponsal')
                            email = (video.corresponsal.nombre, video.corresponsal.email)
                        else:
                            print(u'Sin correpsonsal o sin email: %s' % video.corresponsal)
                            continue
            ...
            for video in videos[:30]:
                print(u"Distribuyendo por FTP video: %s" % video)
                dist = Distribuido(status=1, video=video, distribucion=distribucion)
                ...
                try:
                    filename = '%s_%s_%s.mp4' % (video.pk, video.programa.slug if video.programa else video.slug, video.fecha.strftime('%Y-%m-%d'))
                    ftp.cwd(distribucion.ftp_dir)
                    ftp.storbinary('STOR %s' % filename, open(video.archivo.path, 'rb'))
                except Exception, e:
