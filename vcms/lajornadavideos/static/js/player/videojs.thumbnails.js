(function() {
  var defaults = {
    ...
    parseImageLink = function(imglocation) {
      ...
      var retval = {src:lsrc,w:parseInt(data[2]),h:parseInt(data[3]),x:parseInt(data[0]),y:parseInt(data[1])};
      return retval;
    };
  ...
  videojs.plugin('thumbnails', function(options) {
    ...
    moveListener = function(event) {
      ...
      //Now check which of the cues applsrcies
      ...
      if (setting.src && img.src != setting.src) {
        if (thumbTrack.src) {
          var dir = thumbTrack.src.replace(/\\/g,'/').replace(/\/[^\/]*$/, '');
          img.src = dir + '/' + setting.src;
